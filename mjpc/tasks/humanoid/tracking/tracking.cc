...
void Tracking::ResidualFn::Residual(const mjModel *model, const mjData *data,
  ...
  if (last_key_index < current_index) {
    mju_zero(residual + counter, model->nu + model->na);
    // mju_scl(residual, residual, 1.0 / (current_index - last_key_index), counter);
    // mju_scl(
    //   residual + model->nu + model->na,
    //   residual + model->nu + model->na,
    //   mju_pow(0.9, current_index - last_key_index),
  ...
  double pelvis_xy_multiplier = 1.0;
  double pelvis_z_multiplier = 1.0;
