...
namespace mjpc::weldedhumanoid
{

  std::string Skateboard::XmlPath() const
  {
    return GetModelPath("skateboard/humanoid_welded/task.xml");
  }
                                        ...
                                        double *residual) const
  {
    ...
    mju_subFrom(board_xy, capture_point, 2);
    double board_distance = mju_norm(board_xy, 2);
    ...
    double plate_distance = mju_abs(back_plate_pos[1] - front_plate_pos[1]);
    double right_feet_y = front_plate_pos[1] - right_feet_slider * plate_distance;


    // calculate y-wise difference between the plates, based on left_feet_slider
    double plate_distance_y = mju_abs(back_plate_pos[0] + 0.2);
    ...
    double left_feet_error = standing*mju_sqrt(distance_x*distance_x + y_weight*distance_y*distance_y + y_weight*distance_z*distance_z);
    // if left foot is in front of right foot, add penalty
    if (left_foot_pos[1] < right_foot_pos[1]) {
      left_feet_error *= 2;
    }
    // right feet error, distance to front plate position
    distance_x = mju_abs(right_foot_pos[0] - front_plate_pos[0]);
    ...
    residual[counter++] = foot_error + torso_error;

    if (jiiri++ % 30000 == 0) {
      // print positions
      printf("error_left=%f\n", left_feet_error);
      printf("error_right=%f\n\n", right_feet_error);
      printf("com_difference=%f, %f\n", com_difference[0], com_difference[1]);
